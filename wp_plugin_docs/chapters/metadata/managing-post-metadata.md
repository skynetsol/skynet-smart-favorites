...
The `unique `flag allows you to declare whether this key should be unique. A non unique key is something a post can have multiple variations of, like price. If you only ever want one price for a post, you should flag it `unique `and the `meta_key `will have one value only. 
